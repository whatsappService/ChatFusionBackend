  ...
  if (!bf) {
    bf = await BusinessFeature.create({
      ...
      meta_json: {
        ...
        config: { retries: 3, timeoutMs: 5000 },
        subscriptions: [],
        logs: [],
      },
    });
...
exports.getConfig = async (business_id) => {
  ...
  return {
    enabled: !!bf.enabled,
    secretMasked: bf.meta_json.secret ? "********" : null,
...
exports.updateConfig = async (business_id, patch) => {
  ...
  if (config && typeof config === "object")
    bf.meta_json.config = { ...bf.meta_json.config, ...config };
  await bf.save();
...
exports.createSubscription = async (
  ...
  if (!event || !url) {
    const e = new Error("event and url are required");
    e.status = 400;
    throw e;
  }
  const { bf } = await getOrInitBF(business_id);
...
exports.deleteSubscription = async (business_id, id) => {
  ...
  if (nextSubs.length === subs.length) {
    const e = new Error("Subscription not found");
    e.status = 404;
    throw e;
  }
...
exports.getLog = async (business_id, id) => {
  ...
  if (!log) {
    const e = new Error("Log not found");
    e.status = 404;
    throw e;
  }
  return log;
...
exports.verifyAndRecordIncoming = async (
  business_id,
  body,
  signatureHeader
) => {
